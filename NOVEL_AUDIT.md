...
## 0.2 Required Canon Anchors (quoted text)
- **Rain and bearings** — Part I, Section 2, [Project Gutenberg link](https://www.gutenberg.org/files/201/201-h/201-h.htm#chap02)  
  Quote: “rain (which falls at stated intervals) coming always from the North.”
- **Class/occupation structure** — Part I, Section 3, [Project Gutenberg link](https://www.gutenberg.org/files/201/201-h/201-h.htm#chap03)  
  Quote: “Our Soldiers and Lowest Classes of Workmen are Triangles.”
- **Fog and sight recognition** — Part I, Section 6, [Project Gutenberg link](https://www.gutenberg.org/files/201/201-h/201-h.htm#chap06)  
...
- **Assumption/extrapolation**: spatial fog micro-variation within a town, randomized interval jitter around a stated mean period, deterministic per-entity job assignment within rank-compatible sets, and collapsing many-coloured sides into a single painted stroke colour when painting is enabled.
...
- **Neo-therapeutic gymnasium (high mortality, courtesy promotion)**  
  ...
  - Implementation: `src/systems/neoTherapySystem.ts` with deterministic enrollment, timed institutional isolation, low survival by default (~26% baseline, boosted deterministically under priest scarcity), and promotion to NearCircle/Priest by courtesy.
...
| Policy regime + inspection campaigns | Part I, Sec. 10; Part I, Sec. 12; Part II, Sec. 22 | Directly stated institutions, with trigger heuristics as implementation assumptions | State policy now reacts after a warmup period to sustained civic pressure from irregularity or overcrowding; active `agitation`, `suppression`, and `cooldown` phases affect shelter urgency, inspection severity, conception pressure, and crowd stress, and the UI shows both transition events and continuous active-phase spans. | `src/systems/policyRegimeSystem.ts`; `src/systems/inspectionSystem.ts`; `src/core/policy.ts`; `src/ui/policyTimelineStore.ts`; `src/render/eventTimelineRenderer.ts` | `policyRegimeEnabled=true`; trigger floors plus hysteresis (`policyTriggerIrregularShare`, `policyTriggerIrregularDelta`, `policyTriggerOvercrowding`, `policyTriggerOvercrowdingDelta`, `policyTriggerPersistenceTicks`, `policyWarmupTicks`) | `tests/policyRegimeSystem.test.ts`, `tests/policyTimelineStore.test.ts`, `tests/inspectionSystem.test.ts`, `tests/policyActivationSmoke.test.ts` |
...
- Triangle inspector now exposes the inherited brain angle directly, which is the canonical heredity variable for isosceles ascent.
...
3. In normal runs, visible ascent is rare because the default isosceles cohort begins far below 60°; a 4.6° lineage still needs roughly 111 male generations to regularize.
...
2. **Expose fog comparative-dimness explanation directly in 1D panel legend** (currently mostly implicit).  
   - Novel anchor: Part I, Section 6.
3. **Explicit irregularity policy mode labels** (lenient vs severe state) without changing mechanics.  
...
| Mechanic / Feature | Novel anchor (Part/Section) | Canon status | User-visible behavior | Core implementation (files + key functions) | Config/UI controls (keys/defaults/where) | Tests / verification |
|---|---|---|---|---|---|---|
| Rain-driven shelter motivation + door-latch entry | Part I, Section 2 (*Of the Climate and Houses in Flatland*) | Strongly implied by the novel | During rain, outside entities strongly seek doors; door contact near the correct side triggers indoor transition. In towns, houses also serve as bearings landmarks, so rain sheltering can target the nearest eligible door even when it is not the current visual hit. Residents remain indoors while rain is active and wait for bearing mainly in desolate/no-guidance cases. | `/Users/fdasaro/Desktop/Flatlander/src/systems/rainSystem.ts` `update`; `/Users/fdasaro/Desktop/Flatlander/src/systems/socialNavMindSystem.ts`; `/Users/fdasaro/Desktop/Flatlander/src/systems/civicOrderSystem.ts`; `/Users/fdasaro/Desktop/Flatlander/src/systems/socialNavSteeringSystem.ts`; `/Users/fdasaro/Desktop/Flatlander/src/systems/houseSystem.ts` `collectDoorContacts` / `enterHouse` / `shouldExitHouse` | `housesEnabled=true`, `rainEnabled=true`, `rainPeriodTicks`, `rainDurationTicks`; UI: *World / Environment* panel | `/Users/fdasaro/Desktop/Flatlander/tests/rainShelterIntention.test.ts`; `/Users/fdasaro/Desktop/Flatlander/tests/rainShelterMajority.test.ts`; `/Users/fdasaro/Desktop/Flatlander/tests/civicOrderSystem.test.ts`; `/Users/fdasaro/Desktop/Flatlander/tests/housingDoorEntryFromCollision.test.ts`; `/Users/fdasaro/Desktop/Flatlander/tests/housingWallFollow.test.ts`; `/Users/fdasaro/Desktop/Flatlander/tests/housingExit.test.ts` |
| Rain intervals with deterministic jitter around stated periods | Part I, Section 2 (“stated intervals”) | Implementation assumption / extrapolation (canon-compatible scaling) | Rain starts/ends remain periodic on average but vary per seed and run state deterministically | `/Users/fdasaro/Desktop/Flatlander/src/systems/rainSystem.ts` `jitteredTicks`; `/Users/fdasaro/Desktop/Flatlander/src/core/world.ts` rain jitter config defaults | `rainBasePeriodTicks=2000`, `rainPeriodJitterFrac=0.24`, `rainBaseDurationTicks=700`, `rainDurationJitterFrac=0.18`; UI uses same weather toggles | `/Users/fdasaro/Desktop/Flatlander/tests/midrunSmoke.test.ts`; `/Users/fdasaro/Desktop/Flatlander/src/tools/midRunAudit.ts` |
| House transition events with reasons (`houseEnter`/`houseExit`) | Part I, Section 2 (doors + bearings) | Implementation assumption / extrapolation (canon-aligned instrumentation) | Timeline/legend now shows explicit house entry/exit events, door side, and deterministic reasons (e.g., `RainShelter`, `WaitForBearing`) | `/Users/fdasaro/Desktop/Flatlander/src/core/events.ts`; `/Users/fdasaro/Desktop/Flatlander/src/systems/houseSystem.ts` (`classifyEnterReason`, `classifyExitReason`, `enterHouse`, `exitHouse`); `/Users/fdasaro/Desktop/Flatlander/src/ui/eventAnalytics.ts`; `/Users/fdasaro/Desktop/Flatlander/src/render/eventTimelineRenderer.ts`; `/Users/fdasaro/Desktop/Flatlander/src/ui/legendModel.ts` | Timeline toggles: `enter house`, `exit house`; Event Highlights + Legend panels | `/Users/fdasaro/Desktop/Flatlander/tests/houseEvents.test.ts`; `/Users/fdasaro/Desktop/Flatlander/tests/eventAnalytics.test.ts`; `/Users/fdasaro/Desktop/Flatlander/tests/eventsEffects.test.ts` |
| God-view rain/fog environment overlays from immutable frame snapshot | Part I, Section 2 (rain/climate) + Section 6 (fog dimness) | Implementation assumption / extrapolation (visualization layer only) | Rain/fog are visibly rendered in God-view without mutating simulation: deterministic rain streaks + fog haze + HUD labels | `/Users/fdasaro/Desktop/Flatlander/src/ui/frameSnapshot.ts`; `/Users/fdasaro/Desktop/Flatlander/src/main.ts` (`captureFrameSnapshot` in `frame`); `/Users/fdasaro/Desktop/Flatlander/src/render/canvasRenderer.ts` (`drawEnvironmentOverlay`) | UI defaults ON: `Show Rain Overlay`, `Show Fog Overlay` in *World / Environment* panel | `/Users/fdasaro/Desktop/Flatlander/tests/frameSnapshot.test.ts` |
| Spatial fog-density field for perception and rendering | Part I, Section 6 (comparative dimness under fog) | Strongly implied by the novel (field shape is assumption) | Fog is denser/lighter by map location; vision dimness and God-view shading use the same deterministic field | `/Users/fdasaro/Desktop/Flatlander/src/core/fogField.ts`; `/Users/fdasaro/Desktop/Flatlander/src/systems/visionSystem.ts`; `/Users/fdasaro/Desktop/Flatlander/src/render/canvasRenderer.ts`; `/Users/fdasaro/Desktop/Flatlander/src/main.ts` (Flatlander view local fog density) | `fogDensity=0.012`, `fogFieldCellSize=84`, `fogFieldVariation=0.45`, `fogTorridZoneStartFrac=0.72`, `fogTorridZoneRelief=0.42` | `/Users/fdasaro/Desktop/Flatlander/tests/fogField.test.ts`; `/Users/fdasaro/Desktop/Flatlander/tests/visionFog.test.ts` |
| Rank-compatible deterministic job/category assignment | Part I, Section 3 and Section 6 (Soldiers/Workmen; Merchant/Physician examples) | Directly stated principle with assignment sampling assumption | Inspector/hover show a stable role label (`Soldier`, `Workman`, `Physician`, `Priest`, etc.) compatible with rank/class | `/Users/fdasaro/Desktop/Flatlander/src/core/jobs.ts`; `/Users/fdasaro/Desktop/Flatlander/src/core/factory.ts`; `/Users/fdasaro/Desktop/Flatlander/src/ui/uiController.ts`; `/Users/fdasaro/Desktop/Flatlander/src/main.ts` | No gameplay config; pure hash on `(worldSeed, entityId)` with rank-compatible pools | `/Users/fdasaro/Desktop/Flatlander/tests/jobs.test.ts` |
| Ancient painting toggle (monochrome default, painted strokes when allowed) | Part I, Section 8 (*Of the Ancient Practice of Painting*) | Directly stated principle with a rendering abstraction assumption | By default both God-view and the individual 1D view remain black-and-white. When `Allow Colour` is enabled, ordinary polygons may show deterministic painted stroke colours; women and priests remain colourless, and the 1D panel can optionally display those painted strokes instead of grayscale. | `/Users/fdasaro/Desktop/Flatlander/src/core/world.ts` (`colorEnabled`); `/Users/fdasaro/Desktop/Flatlander/src/render/painting.ts`; `/Users/fdasaro/Desktop/Flatlander/src/render/canvasRenderer.ts`; `/Users/fdasaro/Desktop/Flatlander/src/render/flatlanderScan.ts`; `/Users/fdasaro/Desktop/Flatlander/src/render/flatlanderViewRenderer.ts`; `/Users/fdasaro/Desktop/Flatlander/src/ui/frameSnapshot.ts` | `colorEnabled=false` by default; UI: *World / Environment* `Allow Colour`; 1D panel `Use Greyscale` remains available only when colour is allowed | `/Users/fdasaro/Desktop/Flatlander/tests/painting.test.ts`; `/Users/fdasaro/Desktop/Flatlander/tests/entityStyle.test.ts`; `/Users/fdasaro/Desktop/Flatlander/tests/frameSnapshot.test.ts`; `/Users/fdasaro/Desktop/Flatlander/tests/flatlanderScan.test.ts` |
| Pregnant women monochrome cue + layered visibility | Part I, Section 4 (women-specific social treatment context) | Implementation assumption / extrapolation | Women with active pregnancy render with a persistent monochrome accent stroke that remains visible even under selection, hover, fog preview, or kill-stroke overlays; HP/age dimming is composed centrally before those overlays. | `/Users/fdasaro/Desktop/Flatlander/src/render/canvasRenderer.ts` pregnancy accent stroke; `/Users/fdasaro/Desktop/Flatlander/src/render/viewModifiers.ts` alpha composition helpers | No separate toggle; age/HP dimming defaults ON in Event Highlights, pregnancy cue shown in God-view entities | `/Users/fdasaro/Desktop/Flatlander/tests/viewModifiers.test.ts`; pregnancy state source covered in `/Users/fdasaro/Desktop/Flatlander/tests/reproduction.test.ts` |
...
| Baseline regular-square population + square continuity | Part I, Section 3 (Squares and Pentagons as “Professional” class) | Directly stated principle with quantity as simulation assumption | Default scenario now includes a visible regular-square cohort; newborns inherit SocialNav movement so shelter and social behavior remain active across generations | `/Users/fdasaro/Desktop/Flatlander/src/presets/defaultScenario.ts` `defaultSpawnPlan`; `/Users/fdasaro/Desktop/Flatlander/src/systems/reproductionSystem.ts` `newbornMovement` | Spawn defaults include regular and irregular squares; newborn SocialNav parameters are fixed constants in `reproductionSystem` | `/Users/fdasaro/Desktop/Flatlander/tests/defaultScenario.test.ts`; `/Users/fdasaro/Desktop/Flatlander/tests/reproduction.test.ts` |
| Environment-reactive BDI (physical + social conditions) | Part I, Section 2 (rain/houses/bearings), Section 4 (crowd danger), Section 6 (fog-limited sight) | Strongly implied by the novel (weights/thresholds are assumptions) | Intention scoring now reacts to south-zone exposure, crowd pressure, and local fog; shelter motivation is elevated during rain and heavy crowding, with deterministic northward bias in risky southern zones | `/Users/fdasaro/Desktop/Flatlander/src/systems/socialNavMindSystem.ts` (`southZoneRisk`, `crowdPressure`, `fogCaution`, intention scoring/goaling); `/Users/fdasaro/Desktop/Flatlander/src/core/housing/shelterPolicy.ts` `shouldSeekShelter`; `/Users/fdasaro/Desktop/Flatlander/src/systems/socialNavSteeringSystem.ts` shelter approach speeds | Uses existing world config knobs: `southAttraction*`, `crowdComfortPopulation`, `crowdStressEnabled`, `fogDensity/fogField*`, `rainEnabled`; no extra UI controls required | `/Users/fdasaro/Desktop/Flatlander/tests/socialNavEnvironmentAdaptation.test.ts`; `/Users/fdasaro/Desktop/Flatlander/tests/rainShelterIntention.test.ts`; `/Users/fdasaro/Desktop/Flatlander/tests/rainShelterMajority.test.ts` |
...
- Environment overlays are adapter-only and snapshot-driven; they visualize weather/fog without affecting simulation state.
