...
| Policy regime + inspection campaigns | Part I, Sec. 10; Part I, Sec. 12; Part II, Sec. 22 | Directly stated institutions, with trigger heuristics as implementation assumptions | State policy now reacts to sustained spikes over baseline rather than default startup composition; active `agitation`, `suppression`, and `cooldown` phases affect shelter urgency, inspection severity, conception pressure, and crowd stress, and the UI shows both transition events and continuous active-phase spans. | `src/systems/policyRegimeSystem.ts`; `src/systems/inspectionSystem.ts`; `src/core/policy.ts`; `src/ui/policyTimelineStore.ts`; `src/render/eventTimelineRenderer.ts` | `policyRegimeEnabled=true`; trigger floors plus hysteresis (`policyTriggerIrregularShare`, `policyTriggerIrregularDelta`, `policyTriggerOvercrowding`, `policyTriggerOvercrowdingDelta`, `policyTriggerPersistenceTicks`) | `tests/policyRegimeSystem.test.ts`, `tests/policyTimelineStore.test.ts`, `tests/inspectionSystem.test.ts` |
