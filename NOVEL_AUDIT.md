# NOVEL_FAITHFULNESS AUDIT (Flatlander v0.9.6)
...
- **Assumption/extrapolation**: spatial fog micro-variation within a town, randomized interval jitter around a stated mean period, and deterministic per-entity job assignment within rank-compatible sets.
...
| Pregnant women color cue + layered visibility | Part I, Section 4 (women-specific social treatment context) | Implementation assumption / extrapolation | Women with active pregnancy render with a persistent accent stroke that remains visible even under selection, hover, fog preview, or kill-stroke overlays; HP/age dimming is composed centrally before those overlays. | `/Users/fdasaro/Desktop/Flatlander/src/render/canvasRenderer.ts` pregnancy accent stroke; `/Users/fdasaro/Desktop/Flatlander/src/render/viewModifiers.ts` alpha composition helpers | No separate toggle; age/HP dimming defaults ON in Event Highlights, pregnancy cue shown in God-view entities | `/Users/fdasaro/Desktop/Flatlander/tests/viewModifiers.test.ts`; pregnancy state source covered in `/Users/fdasaro/Desktop/Flatlander/tests/reproduction.test.ts` |
